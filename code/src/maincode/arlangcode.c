...
int main() {
    ...
    while (1) {
        ...
        if (strcmp(command, "print") == 0) {
        ...
        } else if (strcmp(command, "addset") == 0) {
            add_to_set(argument);
        } else if (strcmp(command, "checkset") == 0) {
            check_in_set(argument);
        } else if (strcmp(command, "remove") == 0) {
            remove_from_set(argument);
        } else if (strcmp(command, "countset") == 0) {
            count_set();
            ...
            execute_operator(input);
        } else if (strcmp(command, "save") == 0) {
            if (argument != NULL) {
                char filename[100];
                snprintf(filename, sizeof(filename), "%s.arl", argument);
                char code[MAX_PROGRAM_SIZE];
                printf("Enter your program code (end with 'END'):\n");
                char *code_ptr = code;
                while (1) {
                    fgets(code_ptr, sizeof(code) - (code_ptr - code), stdin);
                    if (strstr(code_ptr, "END") != NULL) {
                        *code_ptr = '\0'; // Завершаем строку
                        break;
                    }
                    code_ptr += strlen(code_ptr);
                }
                save_program(filename, code);
            } else {
                printf("Usage: save(<program_name>)\n");
            }
